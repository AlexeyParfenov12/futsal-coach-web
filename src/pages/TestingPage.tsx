...
export default function TestingPage() {
  ...
  const [playerId, setPlayerId] = useState(params.get('player') || data.players[0]?.id || '');
  ...
  return (
    <>
      <header className="page-header">
        <div><span className="kicker">ВВОД РЕЗУЛЬТАТОВ</span><h1>Физическое тестирование</h1><p>Заполняй только то, что реально измерил. Остальные поля рассчитываются автоматически.</p></div>
      </header>
        ...
        <section className="panel test-config">
          ...
          <label className="field">
            ...
            <select value={playerId} onChange={e=>setPlayerId(e.target.value)}>
              {data.players.filter(p=>p.status==='Активен').map(p=><option key={p.id} value={p.id}>{p.name} · {p.position}</option>)}
            </select>
          </label>
          ...
          <div className="form-row">
            <label className="field">
              ...
              <select value={type} onChange={e=>setType(e.target.value as 'Контрольное'|'Мониторинг')}>
                <option>Контрольное</option><option>Мониторинг</option>
              </select>
            </label>
            <label className="field">
              <span>Метод измерения</span>
              <select value={method} onChange={e=>setMethod(e.target.value)}>
                <option>Видео 240 fps</option><option>Видео 120 fps</option><option>Фотоэлементы</option><option>Другое</option>
              </select>
            </label>
          </div>
